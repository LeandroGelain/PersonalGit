...
def contMember(key, token, idMember):
	url = "https://api.trello.com/1/members/"+str(idCard)+"/cards"
	querystring = {"filter":"visible","key":key,"token":token}
	response = requests.request("GET", url, params=querystring)
	contMembers = (json.loads(response.content.decode('utf-8')))
	print (contMembers)
